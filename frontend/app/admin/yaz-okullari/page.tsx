...
export default function YazOkullariPage() {
  ...
  const handleDeleteConfirm = async () => {
    ...
    setDeleting(true);
    try {
      const res = await fetch(`${API_BASE_URL}${API_ENDPOINTS.summerSchoolById(pendingDeleteId)}`, {
        method: 'DELETE',
        headers: getAuthHeaders(false),
      });
      if (!res.ok) throw new Error('Silme başarısız.');
      setYazOkullari(prev => prev.filter(item => item.id !== pendingDeleteId));
      ...
      {pendingDeleteId != null && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={() => !deleting && setPendingDeleteId(null)}>
          <div className="bg-white rounded-xl shadow-xl border-2 border-gray-200 p-6 max-w-sm w-full" onClick={(e) => e.stopPropagation()}>
            <p className="text-gray-800 font-semibold mb-4">Bu yaz okulunu silmek istediğinize emin misiniz?</p>
