...
export default function StajDuzenlePage() {
  ...
  const addProgram = () => setPrograms(prev => [...prev, { title: '', description: '', duration: '', areas: [''], requirements: [''] }]);
  const removeProgram = (index: number) => setPrograms(prev => prev.filter((_, i) => i !== index));
  ...
  useEffect(() => {
    const fetchProgram = async () => {
      ...
      try {
        ...
        setPrograms(data.programs && data.programs.length > 0 ? data.programs : [{ title: '', description: '', duration: '', areas: [''], requirements: [''] }]);
        setAdvantages(data.advantages && data.advantages.length > 0 ? data.advantages : ['']);
  ...
  if (error) {
    return (
      <div className="space-y-6">
        <div className="bg-white rounded-xl shadow-lg border-2 border-gray-200 p-6 text-center text-red-600">
          {error}
        </div>
